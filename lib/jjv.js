...
//           copy = Object.create(Object.getPrototypeOf(obj));
  ...
  var checkValidity = function (env, schema_stack, object_stack, options) {
    ...
    //support for draft-03 keyword extends.
    if (schema.hasOwnProperty('extends')) {
      objerr = checkValidity(env, schema_stack.concat(schema['extends']), object_stack, options);
      if (objerr)
        return objerr;
    }
    ...
    if (!Array.isArray(prop)) {
      ...
      //HACK: support for draft-03 version of required.
      //TODO: drop jjv or upgrade
      //to draft-04 tooling, -kobrien
      if (schema.hasOwnProperty('properties')) {
        for (var childProp in schema.properties) {
            if (schema.properties[childProp].required === true) {
              if (!prop.hasOwnProperty(childProp)) {
                objerrs[childProp] = {'required': true};
                malformed = true;
              }
            }
        }
